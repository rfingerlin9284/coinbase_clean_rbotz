...
def check_broker_tradability(
    ...
    try:
        ...
        # ── 1. Live quote fetch (Coinbase Native) ─────────────────────────
        if not hasattr(connector, "get_price"):
            return {
                "allowed": False,
                "event":   GATE_ERROR,
                "detail":  {"symbol": symbol, "error": "Connector lacks get_price"}
            }

        px = connector.get_price(symbol)
        if px <= 0:
            return {
                "allowed": False,
                "event":   QUOTE_FETCH_FAILED,
                "detail":  {"symbol": symbol, "response": "Price returned 0 or fetch failed"},
            }

        price_obj = {"tradeable": True}
        quote_time_str = datetime.now(_tz.utc).isoformat()
        
        # Coinbase Spot doesn't expose active bid/ask widely in get_price,
        # so we assume tight spreads for majors during tradability check.
        spread_pips = 0.0
        bid = px
        ask = px
