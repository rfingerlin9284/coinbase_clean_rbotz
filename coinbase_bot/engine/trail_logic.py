...
PAIR_CLASS = {
    ...
    # Crypto — major caps get 'minor' class thresholds (step1: 0.06%, trail: 0.18%)
    # which suits their ~0.5-2% intraday swings. Alts fall through to 'exotic'.
    "BTC-USD": "minor", "ETH-USD": "minor",
    "SOL-USD": "exotic", "AVAX-USD": "exotic", "LINK-USD": "exotic",
    "DOT-USD": "exotic", "POL-USD": "exotic", "ADA-USD": "exotic",
    "DOGE-USD": "exotic", "SHIB-USD": "exotic", "UNI-USD": "exotic",
    "AAVE-USD": "exotic", "XRP-USD": "exotic", "LTC-USD": "exotic",
    "ATOM-USD": "exotic", "NEAR-USD": "exotic", "FIL-USD": "exotic",
    "ALGO-USD": "exotic", "XLM-USD": "exotic",
}
    ...
    *,
    ...
    if not meta.get("tight_step1", False):
        ...
        if (side == "BUY" and price >= tgt) or (side == "SELL" and price <= tgt):
            # Minimum lock clears spread — price-proportional for crypto
            min_lock_pips = entry * 0.0003 if entry > 0 else 0.0003
            if side == "BUY":
