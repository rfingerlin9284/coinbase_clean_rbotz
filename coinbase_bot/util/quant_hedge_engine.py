...
class QuantHedgeEngine:
    ...
    def __init__(self) -> None:
        ...
        # CRYPTO NOTE: Most major crypto pairs (BTC, ETH, SOL, etc.) are
        # positively correlated (0.6-0.9). Placing "inverse" hedges from
        # a forex-style map would INCREASE directional exposure, not reduce it.
        # This map is intentionally empty until true inverse-correlated crypto
        # pairs are identified through backtesting.
        # The engine handles missing hedges gracefully (returns None, skips).
    ...
    def execute_hedge(
        ...
        hedge_size = max(1000.0, round(abs(float(position_size)) * ratio, -3))  # round to nearest 1000 units
