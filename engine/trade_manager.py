...
RBOTZILLA_COINBASE_CLEAN
Label: CRYPTO_ALIGNED_WITH_OANDA

Manages existing Coinbase trades (locally — no broker-side SL/TP):
  - Position sync (new/closed detection)
  - Local SL enforcement (naked execution model)
  - Green-lock SL enforcement (profit protection)
  - Hard dollar stop (env-configurable)
  - RBZ Two-Step SL lock + aggressive trailing (via trail_logic)
  - Counter-trend H4 trail speed reduction
  - ATR-based dynamic trail calibration
  - Stagnation kill-switch (tighten SL after N idle cycles)
  - SL rejection backoff (60s cooldown per price)
  - Per-pair win/loss stats tracker
  - Heartbeat tracking
...
class TradeManager:
    ...
    def manage_open_trades(self, engine_positions: dict = None) -> None:
        ...
        for trade in open_trades:
            ...
            # ── 3-LOCAL. Naked SL enforcement (crypto has no broker-side SL) ──
            local_sl = self._managed.get(trade_id, {}).get("current_sl")
            if local_sl and local_sl > 0:
                sl_hit = (
                    (direction == "BUY" and current_price <= local_sl)
                    or (direction == "SELL" and current_price >= local_sl)
                )
                if sl_hit:
                    try:
                        self.broker.close_trade(trade_id)
                        self._managed.pop(trade_id, None)
                        print(f"  [MANAGER] 🛑 LOCAL_SL_HIT  {instrument}"
                              f"  price={current_price:.5f} crossed SL={local_sl:.5f}  CLOSED")
                        continue
                    except Exception as sl_err:
                        print(f"  [MANAGER] ⚠️  Local SL close failed for {instrument}: {sl_err}")

            # ── 3a. Scale-Out at 1R ──────────────────────────────────────────
            managed_state = self._managed.get(trade_id, {})
            initial_sl = managed_state.get("initial_sl", entry)
            # Provide the actual units
            current_units = abs(float(units))
            if self._try_scale_out_1r(
                trade_id, instrument, direction, entry, current_price, current_sl, current_units
            ):
            ...
            # ── Counter-trend detection & Tight Trail (DISABLED) ──
            # Disabled by Operator: Let the trades breathe to hit full TP.
            pass
            try:
                # Persist meta back
            ...
            pip_size = current_price * 0.001 if current_price > 0 else 0.0001
            ...
            if tid not in broker_ids:
                ...
                if _close_entry and inst:
                    try:
                        ...
                        if _close_price:
                            _close_pnl = ((_close_price - _close_entry) if _close_dir == "BUY"
                                          else (_close_entry - _close_price))
