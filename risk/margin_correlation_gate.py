...
class MarginCorrelationGate:
    ...
    def split_symbol(symbol: str) -> Tuple[str, str]:
        """Split 'EUR_USD' or 'BTC-USD' → ('EUR'/'BTC', 'USD')"""
        parts = symbol.replace("/", "_").replace("-", "_").split("_")
        if len(parts) == 2:
    ...
    def validate_stop_loss_distance(
        ...
        Returns:
            (is_valid, reason)
        """
        # Crypto adaptation: pip distance is price-proportional (0.1% of entry)
        pip_size = entry_price * 0.001 if entry_price > 0 else 0.0001
