...
def _pip_size(symbol: str, price: float = 0.0) -> float:
    """Return pip-equivalent unit for the symbol.
    
    Crypto adaptation: uses 0.1% of price as the base unit.
    For BTC at $65K → pip = $65.  For ETH at $3K → pip = $3.
    For sub-dollar tokens → floor at $0.0001 to prevent zero-pip.
    Falls back to forex logic only if price is unavailable.
    """
    if price > 0:
        return max(price * 0.001, 0.0001)
    # Fallback for callers that don't pass price
    # (will be corrected once we see candle data)
    return 0.0001
...
_DEFAULT_SESSIONS = ["tokyo", "london", "new_york"]  # crypto: always in-session
...
def detect_trap_reversal(symbol: str, candles: list) -> Optional[SignalResult]:
    ...
    pip   = _pip_size(symbol, closes[-1] if closes else 0.0)
    price = closes[-1]
...
def scan_symbol(
    ...
    # ── Native Price-Action Regime Router ────────────────────────────────────
    _closes_list = _closes(candles)
    if len(_closes_list) >= 50:
        ema50_now = _ema(_closes_list, 50)
        ema50_old = _ema(_closes_list[:-10], 50)
        slope = abs(ema50_now - ema50_old) / max(ema50_old, 1e-9) * 100
        _regime_label = "trending" if slope > 0.15 else "sideways"
    else:
        _regime_label = "unknown"

    active_detectors = [detect_liquidity_sweep, detect_aggressive_shorting_ob]
    if _regime_label in ("trending", "unknown"):
        active_detectors.extend([
            # detect_momentum_sma,  # DISABLED: Order Block Sniper Active detect_ema_stack, detect_fvg, 
            detect_fibonacci, detect_ema_scalper_200
        ])
    if _regime_label in ("sideways", "unknown"):
        active_detectors.extend([
            detect_trap_reversal, detect_rsi_extremes, detect_mean_reversion_bb
        ])
    ...
    # The Native Price-Action Regime Router (defined above) already filtered out
    # conflicting strategies, so we no longer need an ML block here to deny trades.
